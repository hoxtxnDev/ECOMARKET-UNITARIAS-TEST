...
class GlobalExceptionHandlerTest {
    ...
    void handleValidationExceptionsReturnsBadRequestWithDetails() {
        ...
        assertEquals("El nombre es obligatorio", response.getBody().getDetails().get("nombre"));
    }

    @Test
    void handleYaExisteEnBdExceptionReturnsConflict() {
        var ex = new YaExisteEnBdException("El producto ya existe");

        ResponseEntity<ErrorResponseDTO> response = handler.handleYaExisteEnBdException(ex, request);

        assertEquals(HttpStatus.CONFLICT, response.getStatusCode());
        assertNotNull(response.getBody());
        assertEquals("El producto ya existe", response.getBody().getMessage());
    }

    @Test
    void handleNoExisteEnBdExceptionReturnsNotFound() {
        var ex = new NoExisteEnBdException("Producto no encontrado");
